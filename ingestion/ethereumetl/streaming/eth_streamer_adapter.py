...
from ingestion.blockchainetl.exporters.buffer_exporter import BufferExporter
...
class EthStreamerAdapter:
    def __init__(
        ...
        self._provider_uri_list = provider_uri_list or [settings.ethereum.provider_uri]

        # Build AsyncWeb3 provider
        self._async_provider = get_failover_async_provider_from_uris(
            self._provider_uri_list, timeout=settings.ethereum.rpc_timeout
        )
    ...
    async def _export_blocks_and_transactions(
        ...
        buffer_exporter = BufferExporter(item_types=["block", "transaction"])
        job = ExportBlocksJob(
    ...
    async def _export_receipts_and_logs(
        ...
        exporter = BufferExporter(item_types=["receipt", "log"])
    ...
    async def _extract_token_transfers(receipt_logs: List[EthReceiptLog]) -> List[EthTokenTransfer]:
        exporter = BufferExporter(item_types=["token_transfer"])
    ...
    async def _export_contracts(receipt_logs: List[EthReceiptLog]) -> List[EthContract]:
        exporter = BufferExporter(item_types=["contract"])
